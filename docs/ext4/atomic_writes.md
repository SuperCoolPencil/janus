# 2.10. Atomic Block Writes — The Linux Kernel  documentation
...
# 2.10. Atomic Block Writes[¶](atomic_writes.md#atomic-block-writes "Permalink to this heading")

## 2.10.1. Introduction[¶](atomic_writes.md#introduction "Permalink to this heading")
...
## 2.10.2. Requirements[¶](atomic_writes.md#requirements "Permalink to this heading")
...
## 2.10.3. Multi-fsblock Implementation Details[¶](atomic_writes.md#multi-fsblock-implementation-details "Permalink to this heading")
...
>   EXT4\_GET\_BLOCKS\_ZERO flag to convert the region into a single contiguous
...
The reason is that, atomic writes when issued via `pwritev2()` with the RWF\_ATOMIC
...
## 2.10.4. Handling Split Extents Across Leaf Blocks[¶](atomic_writes.md#handling-split-extents-across-leaf-blocks "Permalink to this heading")
...
## 2.10.5. Handling Journal transactions[¶](atomic_writes.md#handling-journal-transactions "Permalink to this heading")
...
## 2.10.6. How to[¶](atomic_writes.md#how-to "Permalink to this heading")

### 2.10.6.1. Creating Filesystems with Atomic Write Support[¶](atomic_writes.md#creating-filesystems-with-atomic-write-support "Permalink to this heading")

First check the atomic write units supported by block device.
See [Hardware Support](atomic_writes.md#atomic-write-bdev-support) for more details.
...
### 2.10.6.2. Application Interface[¶](atomic_writes.md#application-interface "Permalink to this heading")
...
>   (e.g., the iovcnt parameter for IOV\_ITER). Currently, this is always set to one.

The STATX\_ATTR\_WRITE\_ATOMIC flag in `statx->attributes` is set if atomic
writes are supported.

## 2.10.7. Hardware Support[¶](atomic_writes.md#hardware-support "Permalink to this heading")
...
## 2.10.8. See Also[¶](atomic_writes.md#see-also "Permalink to this heading")

* [Bigalloc](bigalloc.md) - Documentation on the bigalloc feature
